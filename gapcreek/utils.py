...
def read_pt_file(filename, labels = False):
    points = []
    with open(filename, 'r') as file:
        for line in file:
            # Split each line by comma and strip whitespace and newline characters
            parts = line.strip().split(',')
            if len(parts) == 5:  # Ensure there are 5 elements in each line
                # Convert string numbers to appropriate data types
                point_id = int(parts[0])
                x = float(parts[1])
                y = float(parts[2])
                z = float(parts[3])
                label = parts[4]
                # Append a tuple to the points list
                if labels:
                    points.append((point_id, x, y, z, label))
                else:
                    points.append((x, y, z))                   
    return points
